...
class GeminiCost(Base):
    """Per-user cost tracking for Gemini embedding API calls."""

    __tablename__ = "gemini_costs"
    __table_args__ = (
        Index("idx_gemini_costs_user", "user_id"),
        Index("idx_gemini_costs_created", "created_at"),
        Index("idx_gemini_costs_operation", "operation"),
    )
    ...
    model: Mapped[str] = mapped_column(String(100), default="gemini-embedding-001")
    batch_count: Mapped[int] = mapped_column(Integer, default=1)
    cost_metadata: Mapped[str | None] = mapped_column(Text)  # JSON
