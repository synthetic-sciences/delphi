"""
Lightweight MCP stdio proxy for Delphi.

Runs locally as an MCP stdio server and forwards every tool call as
JSON-RPC over HTTP to a Delphi backend. No heavy dependencies required.
...
Get your API key at http://localhost:8742 (run `npx @synsci/delphi` to
spin up a local instance and mint a key).
"""
...
def main():
    """Entry point for the synsci-delphi-proxy CLI."""
    if not API_KEY:
        print(
